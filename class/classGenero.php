...
class genero extends baseDatos
{
    ...
    function ejecuta($p_accion,$p_id=0){
        ...
        switch($p_accion){
            case 'formEdit':
                $registro=$this->getTupla("select * from catagenero where idGenero=".$p_id);
            case 'formNew':
                ...
                if($p_accion=='formNew')
                    $html.="<h3  class='text-center mb-5'>Nuevo Genero</h3>";
                else{
                    $html.='<h3 class="text-center mb-5">Actualizar Genero</h3>
                            <input type="hidden" name="Id" value="'.(isset($registro)?$registro->idGenero:'').'"';
                }               
                ...
                break;
            case 'borrar':
                $query='delete from catagenero where idGenero='.$p_id;
                $this->consulta($query);
                return $this->list();
                break;
            case 'insert';
                $query="insert into catagenero(Nombre) values ('".$_POST['nombre']."')";
                $this->consulta($query);
                return $this->list();
                break;
            case 'update':
                $query='update catagenero set Nombre="'.$_POST['nombre'].'" where idGenero='.$_POST['Id'];
                $this->consulta($query);
                return $this->list();
                break;
            default:
                $html=$p_accion."No esta programada en classGenero";        
        }
        return $html;
    }
}
$objeGenero = new genero();
