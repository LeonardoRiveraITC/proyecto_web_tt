...
class Roles extends baseDatos
{
    function list()
    {
        $this->consulta("SELECT * FROM catarol order by Nombre");
    ...
    function ejecuta($p_accion,$p_id=0){
        ...
        switch($p_accion){
            case 'formEdit':
                $registro=$this->getTupla("select * from catarol where idRol=".$p_id);
            case 'formNew':
                ...
                if($p_accion=='formNew')
                    $html.="<h3  class='text-center mb-5'>Nueva Rol de usuario</h3>";
                else{
                    $html.='<h3 class="text-center mb-5">Actualizar Rol de usuarios</h3>
                            <input type="hidden" name="Id" value="'.(isset($registro)?$registro->idRol:'').'"';
                }               
                ...
                break;
            case 'borrar':
                $query='delete from catarol where idRol='.$p_id;
                $this->consulta($query);
                return $this->list();
                break;
            case 'insert';
                $query="insert into catarol(Nombre) values ('".$_POST['nombre']."')";
                $this->consulta($query);
                return $this->list();
                break;
            case 'update':
                $query='update catarol set Nombre="'.$_POST['nombre'].'" where idRol='.$_POST['Id'];
                $this->consulta($query);
                return $this->list();
                break;
            default:
                $html=$p_accion."No esta programada en classRoles";        
        }
        return $html;
    }
}
